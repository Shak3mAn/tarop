...
import { MapView } from "../../maps/modals/dash-map-view";
import {
  DialogContent,
  DialogHeader,
  DialogTitle,
...
import DashMapSwitcher from "../../switchers/dash-map-switcher";
...
export const DashMapModal = () => {
  ...
  const isTeamDst = useTeamDistance();
  const isTaskDst = useTaskDistance();
  const isDriverDst = useDriverDistance();

  const { isTeamDistance, isTaskDistance } = useDriverDistance();
  const isTaskDstTeam = useTeamDistance((state) => state.isTaskDistance);
  const isDriverDstTeam = useTeamDistance((state) => state.isDriverDistance);
  const isTaskDstTask = useTaskDistance((state) => state.isTaskDistance);
  ...
  useEffect(() => {
    if (isDashMap) {
      ...
      if (isTeamDistance && isDashMap.label === isTeamDistance.driver) {
      ...
      if (isTaskDstTeam && isDashMap.label === isTaskDstTeam.team) {
        if (
      ...
      if (isTaskDstTask && isDashMap.label === isTaskDstTask.task) {
        setMeta({
    ...
    <>
      <DialogPrimitive.Root>
        <DialogPrimitive.Trigger asChild>
          <button
            className={cn(
              buttonVariants({ variant: "mainOutline", size: "mainIcon" })
            )}
          >
                          ...
                          <>
                            ...
                            <div className="ml-2 mt-6 pb-10">
                              <div className="flex pb-3 pt-0 items-center justify-between">
                                ...
                                <DashMapSwitcher items={formattedSwitchers} />
                        ...
                        <div className="">
                          <div className="space-y-4 py-1 pb-4">
                            <div className="space-y-4">
                              ...
                              <div className="pt-10 absolute space-x-2 flex items-end bottom-14 right-10 justify-end w-full">
                                {/* <DialogPrimitive.Close asChild>
                                  <Button disabled={loading} variant="outline">
                                    Cancel
                                  </Button>
