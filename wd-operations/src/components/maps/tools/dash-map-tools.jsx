...
export const DashMapTools = () => {
  ...
  const { fetchTasksMeta, tasks, tasksMeta, tasksMM } = useTasksMapMeta();
  ...
  return (
    <div className="flex absolute z-[50] flex-col bg-transparent space-y-1.5 transition-all ease-in-out duration-300">
      <div
        className={cn(
          "bg-card p-1 rounded-xl shadow-md items-center space-x-3 w-20 justify-start flex transition-all duration-150 ease-in-out hover:scale-110 hover:ml-2 hover:mt-1",
          !tools.isMapPin
      ...
      <div
        className={cn(
          "bg-transparent opacity-90 flex-col hover:opacity-95 duration-500 transition-all ease-in-out",
          tools.isMapPin ? "ml-2 flex shadow-md" : "hidden"
        )}
      >
        <DashMapSwitcher items={formatSwitch} />
      </div>
    </div>
  );
};
