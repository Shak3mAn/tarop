...
export const EditOperatorModal = ({ className, initialData }) => {
  const [loading, setLoading] = useState(false);

  const router = useRouter();

  const { updateOperator } = useOperatorStore();
  ...
  const onSubmit = async (data) => {
    const notificationData = {
      ...
      title: "Operator",
      action: "Updated",
      description: `Operator: ${data.fullName} has been updated`,
    }

    try {
      setLoading(true);
      updateOperator(data.id, data);
          ...
          <DialogHeader className="pt-4 flex flex-row items-center justify-start">
            <div className="flex space-x-2">
              ...
              <div className="flex flex-col">
                ...
                <DialogDescription>
                  Edit Existing Operation Coordinator
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
